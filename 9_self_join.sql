...
SELECT DISTINCT begCraig.num, begCraig.company, endSight.name, endSight.num, endSight.company
FROM
  (SELECT DISTINCT stopb.id, a.company, a.num
  FROM route a JOIN route b ON
    (a.company=b.company AND a.num=b.num)
    JOIN stops stopa ON (a.stop=stopa.id)
    JOIN stops stopb ON (b.stop=stopb.id)
  WHERE stopa.name='Craiglockhart') begCraig
  JOIN
  (SELECT DISTINCT stopc.id, stopc.name, d.company, d.num
  FROM route c JOIN route d ON
    (c.company=d.company AND c.num=d.num)
    JOIN stops stopc ON (c.stop=stopc.id)
    JOIN stops stopd ON (d.stop=stopd.id)
  WHERE stopd.name='Sighthill'
ORDER BY d.num DESC) endSight
  ON begCraig.id = endSight.id
